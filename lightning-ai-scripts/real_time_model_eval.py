...
def main(
    ...
    script_cmd = f"cd {cwd} && torchrun {torchrun_map[selected_machine]} --tee 3 --log-dir {log_dir / run_name} scripts/run_inference.py \\"
    script_args = {
        # NOTE: this has to come before any other "model" args
    ...
    script_args = {**script_args, **run_inference_args}
    script_args_str = " \\\n".join(f"--{k} {v}" for k, v in script_args.items())
    cmd = "\n".join([script_cmd, script_args_str])
    print(cmd)
    print()
    if not dry_run:
        Job.run(
